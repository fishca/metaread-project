...
thread_local TThread* cur_thread = nullptr;
thread_local Value1C_obj_ExportThread* cur_export_thread = nullptr;
thread_local std::vector<UninitValue1C>* puninitvalues = nullptr;

//********************************************************
// Перегрузка операторов

bool operator<(const TGUID& l, const TGUID& r)
{
...
//---------------------------------------------------------------------------
String KindOfValue1C_presantation(KindOfValue1C kv)
{
	switch(kv)
	{
...
//---------------------------------------------------------------------------
void LoadValidValues(tree* tr, std::vector<VarValidValue>& validvalues, bool haveglobal = false)
{
	tree* tt;
	int i, count;
	VarValidValue vvv;
	...
	tt = tr->get_first();
        count = ToIntDef(tt->get_value(), 0);
	for(i = 0; i < count; ++i)
	{
		tt = tt->get_next();
		vvv.value = ToIntDef(tt->get_value(), 0);
		tt = tt->get_next();
		s = tt->get_value();
		if(IsEmpty(s)) vvv.ver1C = v1C_min;
		else
		{
		...
		{
			tt = tt->get_next();
			vvv.globalvalue = ToIntDef(tt->get_value(), 0);
		}
...
//---------------------------------------------------------------------------
MetaValue::MetaValue(MetaType* _owner, tree* tr) : owner(_owner)
{
	tree* tt;
	...
	fename = tt->get_value();
	tt = tt->get_next();
	fvalue = ToIntDef(tt->get_value(), 0);
	tt = tt->get_next();
...
//---------------------------------------------------------------------------
MetaProperty::MetaProperty(MetaType* _owner, tree* tr) : owner(_owner)
{
	tree* tt;
	tree* t;
	int num, i;
	...
	fename = tt->get_value();
	tt = tt->get_next();
	fpredefined = Compare(tt->get_value(), L"1") == 0 ? true : false;
	tt = tt->get_next();
...
//---------------------------------------------------------------------------
void MetaProperty::filltypes()
{
	std::vector<String>::iterator i;
	ftypes.erase(ftypes.begin(), ftypes.end());
	for(i = fstypes.begin(); i != fstypes.end(); ++i) ftypes.push_back(owner->getTypeSet()->getTypeByName(*i));
}

////---------------------------------------------------------------------------
//void MetaProperty::setdefaultvalue(MetaType* typ, const String& valuename)
//{
...
//---------------------------------------------------------------------------
MetaGeneratedType::MetaGeneratedType(tree* tr)
{
	tree* tt;
	...
	fename = tt->get_value();
	tt = tt->get_next();
	fwoprefix = Compare(tt->get_value(), L"1") == 0 ? true : false;
...
//---------------------------------------------------------------------------
MetaRight* MetaRight::getright(const TGUID& _uid)
{
...
//---------------------------------------------------------------------------
MetaRight* MetaRight::getright(const String& _name)
{
...
//---------------------------------------------------------------------------
MetaStandartAttribute::MetaStandartAttribute(tree* tr)
{
	tree* tt;
	...
	fvalue = ToIntDef(tt->get_value(), 0);
	tt = tt->get_next();
	fcount = StringUtils::Compare(tt->get_value(), L"1") == 0 ? true : false;
	tt = tt->get_next();
	fvaluemax = ToIntDef(tt->get_value(), 0);
	tt = tt->get_next();
	string_to_GUID(tt->get_value(), &fuid);
}
...
//---------------------------------------------------------------------------
MetaStandartTabularSection::MetaStandartTabularSection(tree* tr)
{
	tree* tt;
...
//---------------------------------------------------------------------------
ClassParameter::ClassParameter(tree* tr)
{
	tree* tt;
...
ClassParameter* ClassParameter::getparam(const String& paramname)
{
	ClassParameter* p;
...
//---------------------------------------------------------------------------
int Class::getparamvalue(ClassParameter* p)
{
...
//---------------------------------------------------------------------------
Class* Class::getclass(const TGUID& id)
{
...
//---------------------------------------------------------------------------
ExternalFile::ExternalFile(MetaType* _owner, tree* tr)
{
	tree* tt;
	String sval;
	String str;
	...
	tt = tt->get_next();
	relativepath = StringUtils::Compare(tt->get_value(), L"1") == 0 ? true : false;
	tt = tt->get_next();
	name = tt->get_value();
	tt = tt->get_next();
	ext = tt->get_value();
	tt = tt->get_next();
	catalog = StringUtils::Compare(tt->get_value(), L"1") == 0 ? true : false;
	tt = tt->get_next();
	filename = tt->get_value();
	tt = tt->get_next();
	format = (ExternalFileFormat)(ToIntDef(tt->get_value(), 0));
	...
	tt = tt->get_next();
	optional = StringUtils::Compare(tt->get_value(), L"1") == 0 ? true : false;
	tt = tt->get_next();
	...
	tt = tt->get_next();
	havecondition = StringUtils::Compare(tt->get_value(), L"1") == 0 ? true : false;
	if(havecondition)
	{
		tt = tt->get_next();
		condition = (SerializationTreeCondition)(ToIntDef(tt->get_value(), 0));
		...
		str = SubString(sval, 1, 1);
		if(Compare(str, String(L"%")) == 0)
		{
			typeval1 = stv_var;
			str1 = StringUtils::SubString(sval, 2, sval.length() - 1);
		}
		else if(Compare(str, String(L".")) == 0)
		{
			sval = StringUtils::SubString(sval, 2, sval.length() - 1);
			typeval1 = stv_prop;
		...
		else if(Compare(str, String(L"*")) == 0)
		{
			...
			typeval1 = stv_value;
			val1 = NULL;
			i = StringUtils::Pos(sval, L".");
			if(i)
			{
				str = StringUtils::SubString(sval, 1, i - 1);
				typ = MetaTypeSet::staticTypes->getTypeByName(str);
				if(typ)
				{
					str = StringUtils::SubString(sval, i + 1, sval.length() - i);
		...
		{
			str = SubString(sval, 1, 2);
			if(StringUtils::CompareIC(str, L"S'") == 0)
			{
				typeval1 = stv_string;
				str1 = StringUtils::SubString(sval, 3, sval.length() - 2);
			}
			else if(StringUtils::CompareIC(str, L"N'") == 0)
			{
				typeval1 = stv_number;
				num1 = StringUtils::ToIntDef(StringUtils::SubString(sval, 3, sval.length() - 2), 0);
			}
			else if(StringUtils::CompareIC(str, L"U'") == 0)
			{
				typeval1 = stv_uid;
				if(!string_to_GUID(StringUtils::SubString(sval, 3, sval.length() - 2), &uid1))
				{
		...
		{
			typeval2 = stv_var;
			str2 = StringUtils::SubString(sval, 2, sval.length() - 1);
		}
		else if(Compare(str, String(L".")) == 0)
		{
			sval = StringUtils::SubString(sval, 2, sval.length() - 1);
		...
		else
		{
			...
			{
				typeval2 = stv_string;
				str2 = StringUtils::SubString(sval, 3, sval.length() - 2);
			}
			else if(StringUtils::CompareIC(str, L"N'") == 0)
			{
				typeval2 = stv_number;
				num2 = StringUtils::ToIntDef(StringUtils::SubString(sval, 3, sval.length() - 2), 0);
			}
			else if(StringUtils::CompareIC(str, L"U'") == 0)
			{
				typeval2 = stv_uid;
				if(!string_to_GUID(StringUtils::SubString(sval, 3, sval.length() - 2), &uid2))
				{
					error(L"Ошибка загрузки статических типов. Ошибка преобразования УИД в условии в описании внешнего файла"
						, L"Тип", owner->getname()
						, L"УИД", sval);
...
//---------------------------------------------------------------------------
	SerializationTreeVar::SerializationTreeVar(tree* tr)
{
	tree* tt;
	...
	fname = tt->get_value();
	tt = tt->get_next();
	fcolcount = StringUtils::Compare(tt->get_value(), L"1") == 0 ? true : false;
	tt = tt->get_next();
	fisglobal = StringUtils::Compare(tt->get_value(), L"1") == 0 ? true : false;
	tt = tt->get_next();
	fisfix = StringUtils::Compare(tt->get_value(), L"1") == 0 ? true : false;
	tt = tt->get_next();
	ffixvalue = ToIntDef(tt->get_value(), 0);
	tt = tt->get_next();
...
//---------------------------------------------------------------------------
SerializationTreeNode::~SerializationTreeNode()
{
...
//---------------------------------------------------------------------------
SerializationTreeNode::SerializationTreeNode(MetaType* _owner, tree* tr, SerializationTreeNode* _parent, unsigned int _index)
{
	unsigned int i;
	String str;
	String stval;
	...
	type = (SerializationTreeNodeType)(ToIntDef(tr->get_value(), 0));
	tr = tr->get_next();
	stval = tr->get_value();
	tr = tr->get_next();
	sval1 = tr->get_value();
	...
	{
		case stt_const:
			typeval1 = (SerializationTreeValueType)(ToIntDef(stval, 0));
			switch(typeval1)
			{
				case stv_string:
					str1 = sval1;
		...
		case stt_gentype:
			...
			break;
		case stt_cond:
			condition = (SerializationTreeCondition)(ToIntDef(stval, 0));
			if(condition <= stc_min || condition >= stc_max)
			{
		...
		case stt_metaid:
			break;
		case stt_classcol:
			if(Compare(stval, String(L"2")) == 0) classtype = stct_notinlist;
		...
		case stt_class:
			...
			typeval2 = stv_ver1C;
			ver1C2 = stringtover1C(sval2.c_str());
			if(ver1C2 == v1C_min)
			{
...
//---------------------------------------------------------------------------
SerializationTreeNode* SerializationTreeNode::SerializationTree(MetaType* _owner, tree* tr, SerializationTreeNode* parent)
{
	int count, i;
...
//---------------------------------------------------------------------------
String SerializationTreeNode::path() const
{
...
//---------------------------------------------------------------------------
String SerializationTreeNode::typevalpresentation(SerializationTreeValueType typeval)
{
	switch(typeval)
	{
...
//---------------------------------------------------------------------------
MetaType::MetaType(MetaTypeSet* _typeSet, tree* tr) : typeSet(_typeSet)
{
	tree* tt;
	tree* t;
	int num, i;
	...
	tt = tt->get_next();
	fserialization_ver = ToIntDef(tt->get_value(), 0);
	tt = tt->get_next();
	fmetaname = tt->get_value();
	tt = tt->get_next();
	femetaname = tt->get_value();
	tt = tt->get_next();
	fimageindex = ToIntDef(tt->get_value(), 0);
	tt = tt->get_next();
	fprenameindex = ToIntDef(tt->get_value(), 0);
	tt = tt->get_next();
...
//---------------------------------------------------------------------------
void MetaType::setSerializationTree(tree* tr)
{
	tree* t;
	int num, i;
...
//---------------------------------------------------------------------------
void MetaType::fillcollectiontypes()
{
	std::vector<String>::iterator i;
...
//---------------------------------------------------------------------------
MetaProperty* MetaType::getProperty(int index)
{
	return fproperties[index];
}

//---------------------------------------------------------------------------
int MetaType::numberOfProperties()
{
...
//---------------------------------------------------------------------------
MetaValue* MetaType::getValue(int value)
{
...
//---------------------------------------------------------------------------
MetaType* MetaTypeSet::getTypeByUID(const TGUID& u)
{
...
//---------------------------------------------------------------------------
void MetaTypeSet::staticTypesLoad(TStream* str)
{
	int number, i;
	unsigned int j;
	MetaType* mtype;
	...
	// Параметры классов
	number = ToIntDef(tt->get_value(), 0);
	for(i = 0; i < number; ++i)
	{
		tt = tt->get_next();
	...
	for(i = 0; i < number; ++i)
	{
		...
		t = t->get_next();
		prop->defaultvaluetype = (DefaultValueType)(ToIntDef(t->get_value(), 0));
		t = t->get_next();
...
//---------------------------------------------------------------------------
GeneratedType::GeneratedType(tree** ptr, String path)
{
...
//---------------------------------------------------------------------------
Value1C::Value1C(Value1C_obj* _parent) : parent(_parent)
{
	type = NULL;
	kind = kv_unknown;
	index = -1;
...
//---------------------------------------------------------------------------
String Value1C::path(MetaContainer* mc, bool english)
{
	unsigned int i, j;
...
//---------------------------------------------------------------------------
String Value1C::fullpath(MetaContainer* mc, bool english)
{
...
//---------------------------------------------------------------------------
Value1C_bool::Value1C_bool(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_bool;
	type = MetaTypeSet::mt_bool;
};

//---------------------------------------------------------------------------
String Value1C_bool::presentation(bool english)
{
...
//---------------------------------------------------------------------------
String Value1C_bool::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_bool::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	String s;
...
//---------------------------------------------------------------------------
Value1C_string::Value1C_string(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_string;
	type = MetaTypeSet::mt_string;
};

//---------------------------------------------------------------------------
String Value1C_string::presentation(bool english)
{
	return v_string;
}

//---------------------------------------------------------------------------
String Value1C_string::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_string::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	TStringBuilder* sb;
...
//---------------------------------------------------------------------------
Value1C_number::Value1C_number(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_number;
	type = MetaTypeSet::mt_number;
};

//---------------------------------------------------------------------------
String Value1C_number::presentation(bool english)
{
	return v_string;
}

//---------------------------------------------------------------------------
String Value1C_number::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_number::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	str->Write(v_string);
	return true;
}
...
//---------------------------------------------------------------------------
Value1C_number_exp::Value1C_number_exp(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_number_exp;
	type = MetaTypeSet::mt_number;
};

//---------------------------------------------------------------------------
String Value1C_number_exp::presentation(bool english)
{
	return v_string;
}

//---------------------------------------------------------------------------
String Value1C_number_exp::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_number_exp::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	str->Write(v_string);
	return true;
}
...
//---------------------------------------------------------------------------
bool Value1C_number_exp::isempty()
{
	if(!this) return true;
	//return v_number == 0;
	return false;
...
//---------------------------------------------------------------------------
Value1C_date::Value1C_date(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_date;
...
//---------------------------------------------------------------------------
String Value1C_date::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_date::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	String s;
	s = L"'";
...
//---------------------------------------------------------------------------
Value1C_null::Value1C_null(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_null;
	type = MetaTypeSet::mt_null;
};

//---------------------------------------------------------------------------
String Value1C_null::presentation(bool english)
{
	return L"<Null>";
}

//---------------------------------------------------------------------------
String Value1C_null::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_null::isempty()
{
	if(!this) return true;
	return false;
}
...
//---------------------------------------------------------------------------
bool Value1C_null::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	String s;
	s = L"NULL";
	str->Write(s);
...
//---------------------------------------------------------------------------
Value1C_undef::Value1C_undef(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_undef;
	type = MetaTypeSet::mt_undef;
};

//---------------------------------------------------------------------------
String Value1C_undef::presentation(bool english)
{
...
//---------------------------------------------------------------------------
String Value1C_undef::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_undef::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	String s;
...
//---------------------------------------------------------------------------
Value1C_type::Value1C_type(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_type;
...
//---------------------------------------------------------------------------
String Value1C_type::presentation(bool english)
{
...
//---------------------------------------------------------------------------
String Value1C_type::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_type::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	String s;
...
//---------------------------------------------------------------------------
Value1C_uid::Value1C_uid(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_uid;
...
//---------------------------------------------------------------------------
String Value1C_uid::presentation(bool english)
{
	return GUID_to_string(v_uid);
}

//---------------------------------------------------------------------------
String Value1C_uid::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_uid::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
...
//---------------------------------------------------------------------------
Value1C_enum::Value1C_enum(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_enum;
	v_enum = NULL;
};

//---------------------------------------------------------------------------
String Value1C_enum::presentation(bool english)
{
	return v_enum->getname(english);
}

//---------------------------------------------------------------------------
String Value1C_enum::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_enum::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	String s;
...
//---------------------------------------------------------------------------
Value1C_stdattr::Value1C_stdattr(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_stdattr;
...
//---------------------------------------------------------------------------
String Value1C_stdattr::presentation(bool english)
{
	if(v_stdattr)
	{
...
//---------------------------------------------------------------------------
String Value1C_stdattr::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_stdattr::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	String s;
...
//---------------------------------------------------------------------------
Value1C_stdtabsec::Value1C_stdtabsec(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_stdtabsec;
...
//---------------------------------------------------------------------------
String Value1C_stdtabsec::presentation(bool english)
{
...
//---------------------------------------------------------------------------
String Value1C_stdtabsec::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_stdtabsec::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	String s;
...
//---------------------------------------------------------------------------
bool Value1C_stdtabsec::isempty()
{
	if(!this) return true;
...
//---------------------------------------------------------------------------
Value1C_obj::Value1C_obj(Value1C_obj* _parent, MetaContainer* _owner) : Value1C(_parent), owner(_owner){
	kind = kv_obj;
};

//---------------------------------------------------------------------------
String Value1C_obj::presentation(bool english)
{
	Value1C_obj* vo;
	Value1C* v;
	Value1C_enum* ve;
...
//---------------------------------------------------------------------------
String Value1C_obj::valuepresentation(bool english)
{
	Value1C* v;
...
//---------------------------------------------------------------------------
Value1C* Value1C_obj::getproperty(MetaProperty* mp)
{
...
//---------------------------------------------------------------------------
Value1C* Value1C_obj::getproperty(const String& prop)
{
	MetaProperty* pn;
...
//---------------------------------------------------------------------------
bool Value1C_obj::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	String is;
	String npath;
	String _npath;
	...
	{
		ct = NULL;
		if(type) if(type->getcollectiontypes().size() == 1) ct = type->getcollectiontypes()[0];
		if(!isfirst)
		{
			if(v_objpropv.size())
			{
...
//---------------------------------------------------------------------------
bool Value1C_obj::compare(MetaProperty* p, Value1C* v)
{
...
//---------------------------------------------------------------------------
Class* Value1C_obj::getclass(bool immediately)
{
	unsigned int i;
	Class* cl;
...
//---------------------------------------------------------------------------
Value1C_metaobj::Value1C_metaobj(Value1C_obj* _parent, MetaContainer* _owner) : Value1C_obj(_parent, _owner){
	kind = kv_metaobj;
	v_metaobj = NULL;
};

//---------------------------------------------------------------------------
String Value1C_metaobj::presentation(bool english)
{
...
//---------------------------------------------------------------------------
String Value1C_metaobj::valuepresentation(bool english)
{
	Value1C* v;
...
//---------------------------------------------------------------------------
Value1C_metaobj::~Value1C_metaobj()
{
...
//---------------------------------------------------------------------------
bool Value1C_metaobj::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	String is;
	String is2;
	String is3;
	...
	strp->Write(GUID_to_string(v_metaobj->getuid()));
	strp->Write(String(L"\r\n"));
...
//---------------------------------------------------------------------------
Value1C_refobj::Value1C_refobj(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_refobj;
	v_metaobj = NULL;
	v_uid = EmptyUID;
};
...
//---------------------------------------------------------------------------
String Value1C_refobj::presentation(bool english)
{
...
//---------------------------------------------------------------------------
String Value1C_refobj::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_refobj::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
...
//---------------------------------------------------------------------------
Value1C_refpre::Value1C_refpre(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_refpre;
	v_prevalue = NULL;
};
...
//---------------------------------------------------------------------------
String Value1C_refpre::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_refpre::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
...
//---------------------------------------------------------------------------
Value1C_right::Value1C_right(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_right;
	v_right = NULL;
};

//---------------------------------------------------------------------------
String Value1C_right::presentation(bool english)
{
...
//---------------------------------------------------------------------------
String Value1C_right::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
bool Value1C_right::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
...
//---------------------------------------------------------------------------
Value1C_binary::Value1C_binary(Value1C_obj* _parent) : Value1C(_parent){
	kind = kv_binary;
	v_binary = NULL;
};

//---------------------------------------------------------------------------
String Value1C_binary::presentation(bool english)
{
...
//---------------------------------------------------------------------------
String Value1C_binary::valuepresentation(bool english)
{
	return L"";
}

//---------------------------------------------------------------------------
Value1C_binary::~Value1C_binary()
{
	delete v_binary;
}

//---------------------------------------------------------------------------
bool Value1C_binary::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	TStringBuilder* sb;
	String s;
...
//---------------------------------------------------------------------------
Value1C_extobj::Value1C_extobj(Value1C_obj* _parent, MetaContainer* _owner, const String& _path, MetaType* _type, TGUID _metauid)
...
//---------------------------------------------------------------------------
Value1C_extobj::~Value1C_extobj()
{
	//close();
}

//---------------------------------------------------------------------------
void Value1C_extobj::open()
{
	tree *tt, *tte;
...
//---------------------------------------------------------------------------
Value1C* Value1C_extobj::getproperty(MetaProperty* mp)
{
	open();
...
//---------------------------------------------------------------------------
Value1C* Value1C_extobj::getproperty(const String& prop)
{
	open();
...
//---------------------------------------------------------------------------
bool Value1C_extobj::Export(const String& path, TStreamWriter* str, int indent, bool english)
{
	bool needclose;
	bool ret;
...
//---------------------------------------------------------------------------
MetaContainer::MetaContainer(ConfigStorage* _stor, bool _useExternal)
{
	tree* tr;
	tree* t;
...
//---------------------------------------------------------------------------
void MetaContainer::inituninitvalues()
{
	int j;
	unsigned int in;
	Value1C *v;
	...
	for(std::vector<UninitValue1C>::iterator ui = _puninitvalues->begin(); ui != _puninitvalues->end(); ++ui)
	{
		...
		else if(v->kind == kv_stdattr)
		{
			...
			else
			{
				...
				if(mo->getvalue()->type == MetaTypeSet::mt_attribute)
				{
					vt = NULL;
					typ = NULL;
					vo = mo->getvalue();
							...
							if(sa->getuid() == ui->sauid)
							{
								...
								if(j < sa->getvalue() || j > sa->getvaluemax())
								{
									error(L"Ошибка формата потока 212. Ошибка загрузки стандартного реквизита. Значение за пределами допустимого."
										...
										, L"Значение", j
										, L"Минимальное значение", sa->getvalue()
										, L"Максимальное значение", sa->getvalue()max
										, L"Путь", ui->path
										, L"Мета", v->fullpath(this, false));
								}
								break;
				...
				if(!vsa->v_stdattr)
				{
					//if(mo->getvalue()->type->standartattributes.size() == 0)
...
//---------------------------------------------------------------------------
Value1C* MetaContainer::readValue1C(tree** ptr, MetaType* t, Value1C_obj* valparent, const TGUID& metauid, Value1C_stdtabsec*& metats, ClassItem* clitem, String path, bool checkend)
{
	tree* tr;
	tree* tt;
	tree* ttt;
...
//		v->type = t;
		...
		else if(t == MetaTypeSet::mt_bool)
		{
			...
			else if(Compare(s, String(L"0")) != 0 && Compare(s, String(L"1")) != 0)
			{
				error(L"Ошибка формата потока 204. Ошибка получения значения типа Булево."
					, L"Значение", s
					, L"Путь", spath + tr->path());
			}
			vb->v_bool = Compare(s, String(L"0")) != 0;
			*ptr = tr->get_next();
		}
						...
						*ptr = tr->get_next();
						...
						if(uid != t->getuid())
						{
							error(L"Ошибка формата потока 69. UID типа не совпадает с UID загружаемого системного перечисления."
								, L"Загружаемый тип", t->getname()
								, L"UID типа", GUID_to_string(t->getuid())
...
//---------------------------------------------------------------------------
void MetaContainer::loadValue1C(Value1C_obj* v, tree** ptr, const SerializationTreeNode* tn, TGUID metauid, Value1C_stdtabsec* metats, ClassItem* clitem, String& path, bool checkend)
{
	MetaType* t;
	MetaType* nt;
	VarValue* varvalues;
	...
	for(i = 0; i < t->getexternalfiles().size(); ++i)
	{
				...
				case stv_classpar:
					...
					else
					{
						error(L"Ошибка формата потока 128. Ошибка вычисления условия внешнего файла. Класс не определён."
							, L"Загружаемый тип", t->getname()
							, L"Параметр класса", ext->getcl()asspar1->getname()
							, L"Тип значения", vv->kind
							, L"Путь", spath);
						nv1 = -1;
					}
				...
				case stv_classpar:
					...
					else
					{
						error(L"Ошибка формата потока 133. Ошибка вычисления условия внешнего файла. Класс не определён."
							, L"Загружаемый тип", t->getname()
							, L"Параметр класса", ext->getcl()asspar2->getname()
							, L"Тип значения", vv->kind
							, L"Путь", spath);
						nv2 = -1;
					}
		...
		if(StringUtils::CompareIC(ext->name, L"<Мета ИД>") == 0)
		{
			...
			{
				if(((Value1C_metaobj*)v)->v_metaobj) npath += GUID_to_string(((Value1C_metaobj*)v)->v_metaobj->getuid());
				else
				{
		...
		switch(ext->format)
		{
			case eff_servalue:
				...
				prop = ext->prop;
				if(useExternal && !prop->getpredefined())
				{
...
//---------------------------------------------------------------------------
void MetaContainer::recursiveLoadValue1C(Value1C_obj* v, VarValue* varvalues, tree** ptr, const SerializationTreeNode* tn, TGUID& metauid, Value1C_stdtabsec*& metats, ClassItem* clitem, String& path, bool checkend)
{
	MetaType* t;
	TGUID uid;
	tree* tr;
	...
	for(; tn; tn = tn->next)
	{
		switch(tn->type)
		{
			case stt_const:
				...
				switch(tn->typeval1)
				{
					...
					case stv_number:
						if(tr->get_type() == nd_number)
						{
							if(ToIntDef(tr->get_value(), 0) != tn->num1)
							{
			...
			case stt_var:
				...
				if(tr->get_type() == nd_number) setVarValue(tn->str1, t, v, varvalues, clitem, ToIntDef(tr->get_value(), 0), spath);
				else
				{
			...
			case stt_prop:
				...
				if(prop)
				{
					...
					if(vv)
					{
						...
						{
							if(((Value1C_metaobj*)vv)->v_metaobj) uv1 = ((Value1C_metaobj*)vv)->v_metaobj->getuid();
							else
							{
			...
			case stt_elcol:
				nt = tn->typeprop;
				if(!nt) if(t->getcollectiontypes().size() == 1) nt = t->getcollectiontypes()[0];
					...
					case stv_classpar:
						...
						else
						{
							error(L"Ошибка формата потока 120. Ошибка вычисления условия. Класс не определён."
								, L"Загружаемый тип", t->getname()
								, L"Путь ДС", tn->path()
								, L"Параметр класса", tn->getcl()asspar1->getname()
								, L"Тип значения", vv->kind
								, L"Путь", spath + tr->path());
							nv1 = -1;
						}
					...
					case stv_classpar:
						...
						else
						{
							error(L"Ошибка формата потока 121. Ошибка вычисления условия. Класс не определён."
								, L"Загружаемый тип", t->getname()
								, L"Путь ДС", tn->path()
								, L"Параметр класса", tn->getcl()asspar2->getname()
								, L"Тип значения", vv->kind
								, L"Путь", spath + tr->path());
							nv2 = -1;
						}
...
//---------------------------------------------------------------------------
Value1C* MetaContainer::readValue1C(MetaType* nt, const SerializationTreeNode* tn, tree*& tr, Value1C_obj* valparent, const TGUID& metauid, Value1C_stdtabsec*& metats, ClassItem* clitem, String& path)
{
	Value1C* nv;
	Value1C_refobj* vro;
	Value1C_refpre* vrp;
...
//---------------------------------------------------------------------------
void MetaContainer::readPredefinedValues(Value1C_metaobj* v, const int ni, const int ui, const Value1C_obj* vStrings, const String& spath)
{
	Value1C_obj* vo;
	Value1C_obj* vvo;
	Value1C_uid* vu;
...
//---------------------------------------------------------------------------
int MetaContainer::getVarValue(const String& vname, MetaType* t, VarValue* varvalues, ClassItem* clitem, String& path)
{
	if(CompareIC(vname, L"ВерсияКонтейнера") == 0) return (int)containerver;
	if(CompareIC(vname, L"Версия1С") == 0) return (int)ver1C;
	if(CompareIC(vname, L"ВерсияКласса") == 0)
	{
		if(clitem) return clitem->getversion();
...
//---------------------------------------------------------------------------
void MetaContainer::setVarValue(const String& vname, MetaType* t, Value1C_obj* v, VarValue* varvalues, ClassItem* clitem, int value, String& path)
{
	SerializationTreeVar* var;
	unsigned int i, j;
	VarValidValue* vvv;
	...
	if(CompareIC(vname, L"ВерсияКласса") == 0)
	{
		if(clitem)
		{
			...
			error(L"Ошибка формата потока 122. Недопустимое значение переменной."
				...
				, L"Имя переменной", vname
				, L"Значение", value
				, L"Класс", GUID_to_string(clitem->getcl()->getuid())
				, L"Путь", path);
	...
	if(varvalues)
	{
		for(i = 0; i < t->getserializationvars().size(); ++i)
		{
			var = t->getserializationvars()[i];
			if(CompareIC(var->getname(), vname) == 0)
			{
				varvalues[i].value = value;
				varvalues[i].isset = true;
				if(var->getvalidvalues().size() == 0) return;
...
//---------------------------------------------------------------------------
MetaObject* MetaContainer::getMetaObject(const String& n)
{
	std::map<String, MetaObject*>::iterator i;
	i = fsmetamap.find(UpperCase(n));
	if(i == fsmetamap.end())
	{
		i = MetaObject::smap.find(UpperCase(n));
...
//---------------------------------------------------------------------------
MetaObject* MetaContainer::getMetaObject(const TGUID& u)
{
...
//---------------------------------------------------------------------------
PredefinedValue* MetaContainer::getPreValue(const TGUID& u)
{
...
//---------------------------------------------------------------------------
bool MetaContainer::Export(const String& path, bool english, unsigned int thread_count)
{
	String npath;
	Value1C_obj_ExportThread* thr;
	unsigned int i;
...
bool MetaContainer::ExportThread(Value1C_obj* v, const String& path, bool english)
//bool MetaContainer::ExportThread(Value1C_obj* v, String path, bool english)
{
	unsigned int i;
	Value1C_obj_ExportThread* thr;
	bool multithread;
...
//---------------------------------------------------------------------------
Value1C_obj_ExportThread::Value1C_obj_ExportThread(MetaContainer* _owner) : owner(_owner), TThread(false)
{
	busy = 0;
	finish = false;
	singlethread = 0;
...
//---------------------------------------------------------------------------
void Value1C_obj_ExportThread::Execute()
{
	cur_thread = this;
...
//---------------------------------------------------------------------------
Value1C_obj_ExportThread::~Value1C_obj_ExportThread()
{
	delete work;
}
